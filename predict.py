...
def main(args):
    ...
    models = ['vit_base', 'vit_large', 'AlexNet', 'vgg19']
    for videoFile in videoFile_List:
        ...
        if args.model_name == 'vit_base':
            print('prediction by ViT_base model')
            model = vit_base_patch16_224(num_classes=args.num_classes).to(device)
        elif args.model_name == 'vit_large':
            print('prediction by ViT_large model')
            model = vit_large_patch16_224(num_classes=args.num_classes).to(device) 
        elif args.model_name == 'AlexNet':
            print('prediction by AlexNet model')
            model = AlexNet(num_classes=args.num_classes, init_weights=False).to(device)
        elif args.model_name == 'vgg19':
            print('prediction by VGG19 model')
            model = vgg(model_name=args.model_name, num_classes=args.num_classes, init_weights=False).to(device)
        else:
            assert args.model_name in models, "model name should be 'vit_base', 'vit_large', 'vgg19' or 'AlexNet'"
