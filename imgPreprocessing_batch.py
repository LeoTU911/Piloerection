...
def imgPreProcessing_pred(filePath, num_Workers):
    ...
    for video in video_List:
        ...
        if len(frameFile_List) == 4:
            ...
            imgPreProcessing(path0, num_Workers)
            imgPreProcessing(path1, num_Workers) 
            imgPreProcessing(path2, num_Workers) 
            imgPreProcessing(path3, num_Workers) 
        # 1-grid video
        else:
            imgPreProcessing(frame_Path, num_Workers)
...
def main(mode='training', frameFile_Path='./', labelFile_Path=''):
    ...
    if mode == "prediction":
        imgPreProcessing_pred(filePath=frameFile_Path, num_Workers=num_Workers)
    elif mode == "training":
...
frameFile_Path = 'C:\\Piloerection\\data\\demo\\local\\frameImage_pred'
#labelFile_Path = 'C:\\Piloerection\\data\\file'
#classifyImg(labelFile_Path, frameFile_Path, save_interval=1)
main(mode='prediction', frameFile_Path=frameFile_Path)
